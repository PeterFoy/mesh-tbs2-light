...
#define TIMER_ID_SYSTEM_RESET  77
#define TIMER_ID_PROVISIONING   66
#define TIMER_ID_NET_MESH_TIMEOUT 67
...
static void initiate_factory_reset(void)
{
  ...
  gecko_cmd_hardware_set_soft_timer(2 * 32768, TIMER_ID_SYSTEM_RESET, 1);
}
...
#define EXT_UCLI_RECV_SIGNAL 				0x4

void stdin_irq_hook(void) {
	gecko_external_signal(EXT_UCLI_RECV_SIGNAL);
}
...
{
	if (delay_ms)
		gecko_cmd_hardware_set_soft_timer(TIMER_MS_2_TIMERTICK(delay_ms), TIMER_ID_SYSTEM_RESET, 1);
	else
...
void handle_gecko_event(uint32_t evt_id, struct gecko_cmd_packet *evt)
{
  ...
  switch (evt_id) {
    ...
    case gecko_evt_mesh_node_initialized_id:
      ...
      net_mesh_init(pData->provisioned, setup_mesh_timer);
    ...
    case gecko_evt_system_external_signal_id:
    {
      ...
      if (evt->data.evt_system_external_signal.extsignals & EXT_UCLI_RECV_SIGNAL) {
		uint8_t buf[8];
